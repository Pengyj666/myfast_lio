...
/**
 * @brief 将SO(3)旋转表示（四元数）转换为欧拉角（roll, pitch, yaw）
 * 
 * 该函数将一个SO(3)类型的旋转向量（单位四元数）转换为对应的欧拉角表示，
 * 使用的是ZYX旋转顺序（即先绕X轴旋转roll，再绕Y轴旋转pitch，最后绕Z轴旋转yaw）。
 * 欧拉角结果以角度为单位返回。
 * 
 * @param orient 输入的SO(3)旋转向量（四元数形式）
 * @return vect3 类型的欧拉角 [roll, pitch, yaw]，单位为度
 */
vect3 SO3ToEuler(const SO3 &orient) 
{
	...
	double unit = sqx + sqy + sqz + sqw; // 如果已归一化则为1，否则作为校正因子
	double test = q_data[3]*q_data[1] - q_data[2]*q_data[0];

	// 处理北极大值奇点情况（pitch接近+90度）
