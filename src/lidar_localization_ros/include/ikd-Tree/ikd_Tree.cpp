...
void KD_TREE<PointType>::stop_thread(){
    ...
    timeout.tv_sec += 2;
    int join_ret = pthread_timedjoin_np(rebuild_thread, NULL, &timeout);
    if (join_ret == 0) {
        printf("stop_thread: 后台重建线程正常终止");
    } else if (join_ret == ETIMEDOUT) {
        printf("stop_thread: 等待线程终止超时，强制退出");
        pthread_cancel(rebuild_thread);
        pthread_join(rebuild_thread, NULL);
    } else {
        printf("stop_thread: 线程终止失败（错误码：%d）", join_ret);
    }
...
void KD_TREE<PointType>::delete_tree_nodes(KD_TREE_NODE ** root){ 
    ...
    if (pthread_mutex_trylock(&(*root)->push_down_mutex_lock) == 0) {
        pthread_mutex_unlock(&(*root)->push_down_mutex_lock);
        pthread_mutex_destroy(&(*root)->push_down_mutex_lock);
    }       
    delete *root;
...
template <typename PointType>
bool KD_TREE<PointType>::Reset_Tree() {
    stop_thread();
    ...
    try {
        if (search_mutex_counter != 0) {
            printf("Reset_Tree: 仍有 %d 个搜索线程在运行，等待退出...", search_mutex_counter);
            while (search_mutex_counter != 0) {
                pthread_mutex_unlock(&search_flag_mutex);
                usleep(100);
                pthread_mutex_lock(&search_flag_mutex);
        ...
        PointVector().swap(PCL_Storage);         // 点云缓存
        PointVector().swap(Rebuild_PCL_Storage); // 重建用点云缓存
        PointVector().swap(Downsample_Storage);  // 下采样用缓存
        PointVector().swap(Points_deleted);      // 删除点记录
        PointVector().swap(Multithread_Points_deleted); // 多线程删除点记录
        Rebuild_Logger.clear();                  // 重建操作日志
        max_queue_size = 0;                      // 日志队列最大尺寸重置

        Rebuild_Ptr = nullptr;       // 重建指针置空
        rebuild_flag = false;        // 重建标志位重置
        termination_flag = false;    // 终止标志位重置
        search_mutex_counter = 0;    // 搜索计数器重置
        Treesize_tmp = 0;            // 临时树大小重置
        Validnum_tmp = 0;            // 临时有效点数重置
        alpha_bal_tmp = 0.0f;        // 临时平衡因子重置
        alpha_del_tmp = 0.0f;        // 临时删除率重置
        ...
        printf("ikd-Tree 重置成功！所有节点已清除，树恢复初始状态\n");
        return true;
