...
void Preprocess::process(const sensor_msgs::PointCloud2::ConstPtr &msg, PointCloudXYZI::Ptr &pcl_out)
{
  ...
  switch (lidar_type)
  {
    case OUST64:
      {
        pcl::PointCloud<ouster_ros::Point> ouster_orig;
        pcl::fromROSMsg(*msg, ouster_orig);
        oust64_handler(ouster_orig);
        break;
      }

    case VELO16:
      {
        pcl::PointCloud<velodyne_ros::Point> velodyne_orig;
        pcl::fromROSMsg(*msg, velodyne_orig);
        velodyne_handler(velodyne_orig);
        break;
      }
    ...
    case WLR722F:
      {
        pcl::PointCloud<WLR722F_ros::Point> WLR722F_orig;
        pcl::fromROSMsg(*msg, WLR722F_orig);
        WLR722F_handler(WLR722F_orig);
        break;
      }
 ...
 */
void Preprocess::WLR722F_handler(const pcl::PointCloud<WLR722F_ros::Point> & pl_orig)
{
    #if 1
    pl_surf.clear();
    pl_corn.clear();
    ...
    else
    {
        for (int i = 0; i < plsize; i++)
        {
          ...
          if (i % point_filter_num == 0)
          {
            ...
            {
              // if(std::atan2(added_pt.y, added_pt.x)*180/M_PI > 160.0 || std::atan2(added_pt.y, added_pt.x)*180/M_PI < 20.0){
              //     pl_surf.points.push_back(added_pt);
              // }

              double angle = std::atan2(added_pt.y, added_pt.x) * 180 / M_PI;
...
void Preprocess::velodyne_handler(const pcl::PointCloud<velodyne_ros::Point> pl_orig)
{
    pl_surf.clear();
    pl_corn.clear();
    pl_full.clear();
...
void Preprocess::sim_handler(const pcl::PointCloud<pcl::PointXYZI> pl_orig) {
    pl_surf.clear();
    pl_full.clear();
