...
bool VioTracker::IsVioValid() {
  return tf_pose_.ts > 0.0;
}
...
common::Data_VioResult VioTracker::GetVioInLocalXyz(const common::Data_VioResult &_vio_result) {
  ...
  } 
  {
    ...
    if (log_filter.Output(GetNow_Steady())) {
      droslog(LogLevel::INFO, "VioTracker::GetVioInLocalXyz() VIO跟踪状态, VioValid=%d, dts: %.3f, tf_pose.ts: %.3f", 
          is_valid, vio_result.timestamp-tf_pose_.ts, tf_pose_.ts);
    }
  }
  return result;
}
