...
static int s_integ_cnt = 0;
std::mutex s_integ_cnt_mutex;
void add_integ_cnt() {
    std::lock_guard<std::mutex> lock(s_integ_cnt_mutex);
    s_integ_cnt++;
}
void sub_integ_cnt() {
    std::lock_guard<std::mutex> lock(s_integ_cnt_mutex);
...
{
    add_integ_cnt();
    if (get_integ_cnt() > 200 && get_integ_cnt() % 50 == 0) {
        droslog(LogLevel::INFO, "IntegrationBase::ctor() new integration, cnt = %d", get_integ_cnt());
    }
...
IntegrationBase::~IntegrationBase() {
    sub_integ_cnt();
    // if (get_integ_cnt() % 50 == 0) {
    //     droslog(LogLevel::INFO, "IntegrationBase::dtor() delete integration, cnt = %d", get_integ_cnt());
    // }
}
