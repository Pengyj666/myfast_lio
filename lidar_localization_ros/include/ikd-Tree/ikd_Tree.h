...
#define ForceRebuildPercentage 0.2  //没用到
...
public:
    ...
    /**
     * @brief KD树节点结构体，用于存储三维空间中的点及其相关属性和状态信息
     * 
     * 该结构体定义了KD树中每个节点的数据结构，包含点坐标、分割信息、
     * 节点状态标记、统计信息以及树结构指针等成员变量。
     */
    struct KD_TREE_NODE{
        PointType point;                                    // 节点存储的三维点坐标
        uint8_t division_axis;                              // 分割轴标识(0-x轴, 1-y轴, 2-z轴)
        int TreeSize = 1;                                   // 以当前节点为根的子树节点总数
        int invalid_point_num = 0;                          // 子树中被标记删除的节点数量
        int down_del_num = 0;                               // 需要向下传递的删除操作数量
        bool point_deleted = false;                         // 当前节点点数据是否被删除标记
        bool tree_deleted = false;                          // 当前子树是否被删除标记
        bool point_downsample_deleted = false;              // 当前节点点数据是否被下采样删除标记
        bool tree_downsample_deleted = false;               // 当前子树是否被下采样删除标记
        bool need_push_down_to_left = false;                // 是否需要将状态推送至左子树
        bool need_push_down_to_right = false;               // 是否需要将状态推送至右子树
        bool working_flag = false;                          // 节点工作状态标记
        float radius_sq;                                    // 节点覆盖范围的平方半径
        pthread_mutex_t push_down_mutex_lock;               // 状态推送操作的互斥锁
        float node_range_x[2], node_range_y[2], node_range_z[2];   // 节点在各轴上的坐标范围[x_min, x_max]
        ...
        float alpha_del;                                    // 删除操作相关的alpha参数
        float alpha_bal;                                    // 平衡操作相关的alpha参数
    };
...
private:
    ...
    float downsample_size = 0.2f; // 下采样大小 点云体素化大小
    ...
    PointVector Multithread_Points_deleted; // 多线程删除点云
